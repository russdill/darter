...
if ext == "ibs":
    ...
    print("Warning: Unknown extensions, '{}', assuming ibs".format(ext), file=sys.stderr)
    parser = pybis.IBSParser()
    ...
    if 'Diff Pin' in comp:
        for pin_name, diff_info in comp.diff_pin.items():
            ...
            if pin_info.model_name != inv_info.model_name:
                ...
                print('Warning: Cannot handle [Diff Pin] {} in component {} with mixed models'.format(pin_name, name), file=sys.stderr)
                continue
            ...
            for model_name in models:
                if model_name in diff_models:
                    if diff_models[model_name] is None:
                        ...
                        print('Warning: Cannot handle variable vdiff across different [Diff Pin] sets with same model (component {}, pin {})'.format(name, pin_name), file=sys.stderr)
                else:
            ...
            for tbl in model[n]:
                for f in [ 'L_fixture', 'R_dut', 'L_dut', 'C_dut' ]:
                    if f in tbl and tbl[f]:
                        print('Error: {} not supported in {}'.format(f, name), file=sys.stderr)
