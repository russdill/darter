...
def param(n, val, invert=None):
    if isinstance(val, pybis.Range):
        print(f'.param {n}={{modv({val(0)}, {val(1, invert)}, {val(2, invert)})}}')
    else:
        print(f'.param {n}={val}')
...
def ibis_translate(str):
    return str.translate(str.maketrans('<>- #', '____c'))

# Convert a SPICE include file, substituting $<var> for vars in 'tables'
def include(lib, tables=None):
    file = lib
    if sys.path[0]:
        file = f'{sys.path[0]}/{lib}'
    print(Template(open(file, 'r', encoding='utf-8').read()).substitute(tables))
...
if ext == "ibs":
    ...
    print(f"Warning: Unknown extensions, '{ext}', assuming ibs", file=sys.stderr)
    parser = pybis.IBSParser()
...
if unsupported:
    print("The following sections are unsupported", file=sys.stderr)
    print(unsupported, file=sys.stderr)
    raise Exception("Unsupported sections")

ignored = sections & ignored_sections
if ignored:
    print("The following sections are ignored", file=sys.stderr)
...
for name, text in main.header.items():
...
for name, model in iter(main.define_package_model.items()) if 'Define Package Model' in main else []:
    ...
    pm = dict()
    for pin, num in model.pin_mapping.items():
        tmp = dict()
...
for name, comp in iter(main.component.items()) if 'component' in main else []:
    name = ibis_translate(name)

    print(f'.lib {name}')
    print(f'.subckt {name} pad gnd die spec=0')
    ...
    for pin, vals in comp.pin.items():
        ...
        for sub_type in ('signal', 'pin'):
            ...
            print(f'.subckt {name}_{sub_name}_{ibis_translate(sub)} pad gnd die spec=0')
            print(f'* {sub_type} {sub_other}')
            ...
            print(modv_func)
            include('ibis_pkg.inc')
            print(f'.ends {name}_{sub_name}_{ibis_translate(sub)}')
        listed.add(vals.signal_name)
    print('.endl')

    # Track info from diff pin sections
    if 'Diff Pin' in comp:
        for pin_name, diff_info in comp.diff_pin.items():
            pin_info = comp.pin[pin_name]
            inv_info = comp.pin[diff_info.inv_pin]
            if pin_info.model_name != inv_info.model_name:
                # We can't handle that...
                print(f'Warning: Cannot handle [Diff Pin] {pin_name} in component {name} with mixed models', file=sys.stderr)
                continue
            if pin_info.model_name in main.model:
                models = [pin_info.model_name]
            else:
                models = list(main.model_selector[pin_info.model_name].keys())
            for model_name in models:
                if model_name in diff_models:
                    if diff_models[model_name] is None:
                        ...
                        print(f'Warning: Cannot handle variable vdiff across different [Diff Pin] sets with same model (component {name}, pin {pin_name})', file=sys.stderr)
                else:
...
# Process each model
for name, model in iter(main.model.items()) if 'Model' in main else []:
    ...
    if type_diff or type_switch or type_ecl:
        print(f'* Unhandled model type: {model_type}')
        continue
    ...
    print(f'.lib {ibis_translate(name)}')
    print(f'* type - {model_type}')

    for key, mode in iter(model.add_submodel.items()) if 'Add Submodel' in model else []:
        print(f'.lib {outfile} {ibis_translate(key)}')

    print(f'.subckt {ibis_translate(name)} A_signal A_signal_neg A_pcref A_gcref A_puref A_pdref D_enable D_drive D_receive A_extref spec=0 start_on=1')
            ...
            for tbl in model[n]:
                for f in [ 'L_fixture', 'R_dut', 'L_dut', 'C_dut' ]:
                    if f in tbl and tbl[f]:
                        print(f'Error: {f} not supported in {name}', file=sys.stderr)
    ...
    for key, mode in iter(model.add_submodel.items()) if 'Add Submodel' in model else []:
        ...
        print(f'x_{ibis_translate(key)} A_signal A_pcref A_gcref A_puref A_pdref {en} {ibis_translate(key)} spec={{spec}}')

    print(f'.ends {ibis_translate(name)}')
    ...
    if 'Vmeas' in model or 'Vmeas' in model_spec:
        for edge in [ 'rising', 'falling' ]:
            ...
            tables = dict()
            print(f'.subckt {ibis_translate(name)}_Vmeas_{edge} time not_ready spec=0')
            ...
            print(f'.ends {ibis_translate(name)}_Vmeas_{edge}')
    ...
    if (not type_input or vdiff_ac is not None) and not type_source and not type_sink:
        ...
        print(f'.subckt {ibis_translate(name)}_DIFF A_signal_pos A_signal_neg A_pcref A_gcref A_puref A_pdref D_enable D_drive D_receive spec=0 start_on=1 tdelay=1f')
        ...
        print(f'X_pos A_signal_pos 0 A_pcref A_gcref A_puref A_pdref D_enable D_drive D_receive_pos 0 {ibis_translate(name)} spec={{spec}} start_on={{start_on}}')
        print(f'X_neg A_signal_neg 0 A_pcref A_gcref A_puref A_pdref D_enable D_not_drive D_receive_neg 0 {ibis_translate(name)} spec={{spec}} start_on={{start_on}}')

        print(f'.ends {ibis_translate(name)}_DIFF')

        # Create a model that can calculate launch time
        if type_output:
            tables = dict()
            print(f'.subckt {ibis_translate(name)}_DIFF_Vmeas time not_ready spec=0')
            ...
            print(f'.ends {ibis_translate(name)}_DIFF_Vmeas')

    print('.endl')

# Create submodel subcircuits
for name, model in iter(main.submodel.items()) if 'submodel' in main else []:

    print(f'.lib {ibis_translate(name)}')
    print(f'* type - {model.submodel_type}')
...
#        table_names.append('rising_waveform')
        ...
        print(f'* Unhandled submodel type: {model.submodel_type}')
        print('.endl')
        continue

    print(f'.subckt {ibis_translate(name)} A_signal A_pcref A_gcref A_puref A_pdref D_enable spec=0')
...
for name, board in iter(main.begin_board_description.items()) if 'Begin Board Description' in main else []:

    print(f'.lib {ibis_translate(name)}')
    print(f'* Manufacturer: {board.manufacturer}')

    for signal_name, path in board.path_description.items():
        ...
        print(f'.subckt {ibis_translate(name)}_PIN_{ibis_translate(pin_name)} net_0 gnd{nodes}')
        ...
        def process_path(path, n=[0], net='net', r=[0]):
            ...
            for item in path:
                if isinstance(item, list):
                    print(f'R_Fork{refdes(r)} {curr(n)} {curr(n)}_{next_n[0]} 0')
                    process_path(item, next_n, curr(n), r)
                    next_n[0] += 1
                    print('* Endfork')
                elif 'Node' in item:
                    print(f'R{item.node} {curr(n)} {item.node} 0')
                elif 'Pin' in item:
                    ...
                    if item.len == 0:
                        ...
                        if 'L' in item or 'R' in item:
                            c /= 2
                        if c != 0:
                            print(f'C{refdes(r)} {curr(n)} gnd {c}')
                        if 'R' in item:
                            print(f'R{refdes(r)} {curr(n)} {next(n)} {item.r}')
                        if 'L' in item:
                            print(f'L{refdes(r)} {curr(n)} {next(n)} {item.l}')
                        ...
                        for n in [ 'L', 'R', 'C' ]:
                            if n in item:
                                args += ' ' + n + '=' + item[n]
                        o = refdes(r)
                        print(f'.MODEL LTRA_{o} LTRA(len={item.len}{args})')
                        print(f'O{o} {curr(n)} gnd {next(n)} gnd LTRA_{o}')

        process_path(path[1:])

        print(f'.ends {ibis_translate(name)}_PIN_{ibis_translate(pin_name)}')

        print(f'.subckt {ibis_translate(name)}_SIG_{ibis_translate(signal_name)} net_0 gnd{nodes}')
        print(f'x net_0 gnd{nodes} {ibis_translate(name)}_PIN_{ibis_translate(pin_name)}')
        print(f'.ends {ibis_translate(name)}_SIG_{ibis_translate(pin_name)}')
